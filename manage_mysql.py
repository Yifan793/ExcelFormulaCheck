...
# 不支持 MD YM YD
datedif_params = ["Y", "M", "D"]
global InitPath
InitPath = './init.xlsx'
global TestCases
TestCases = "./TestCasesMySql.xlsx"
global AllFormulaTypeTest
AllFormulaTypeTest = "./AllFormulaTypeTest_MySql_part2.xlsx"
global AfterAllFormulaTypeTest_MySql
AfterAllFormulaTypeTest_MySql = "./AfterAllFormulaTypeTest_MySql_part2.xlsx"
                            ...
                            for init_value2 in init_values[value_type2]:
                                if formula_type == "POWER":
                                    ...
                                    if isinstance(init_value1, float) and isinstance(init_value2, float) and init_value1 < 0 and init_value2 % 1 != 0:
                                        continue
                                    # POWER(Text, Negative)第二位不能是负数
                                    if isinstance(init_value1, str) and isinstance(init_value2, float) and init_value2 < 0:
                                        continue
                            ...
                            for init_value1 in init_values[value_type1]:
                                for init_value2 in init_values[value_type2]:
                                    for datedif_param in datedif_params:
                                        # M时通过PERIOD_DIFF(DATE_FORMAT(Type, "%Y%m"), DATE_FORMAT(Type, "%Y%m"))计算
                                        # 报错Incorrect arguments to period_diff
                                        # 因为DATE_FORMAT(0, "%Y%m")的结果是000000，会出现0月报错
                                        # DATE_FORMAT(5/-7, "%Y%m")的结果是null，不会有问题，但Date数据会有问题

                                        # 同时为0时continue，出现Date/Time和0时continue
                                        if isinstance(init_value1, float) and init_value1 == 0 and isinstance(
                                                init_value2, float) and init_value2 == 0:
                                            continue
                                        if (value_type1 == "Date" or value_type1 == "Time") and isinstance(init_value2,
                                                                                float) and init_value2 == 0:
                                            continue
                                        if (value_type2 == "Date" or value_type2 == "Time") and isinstance(init_value1,
                                                                                float) and init_value1 == 0:
                                            continue
...
def after_sql_to_excel():
    global sheet_formula
    sheet_formula = pd.read_excel(InitPath, sheet_name="Formula_MySql")
    ...
    # 保存修改后的Excel文件
    workbook.save(AfterAllFormulaTypeTest_MySql)


def add_conclusion():
    global sheet_formula
    sheet_formula = pd.read_excel(InitPath, sheet_name="Formula")
    workbook1 = load_workbook('./AfterAllFormulaTypeTestResultWithConclusion_MySql.xlsx')
    workbook2 = load_workbook(AfterAllFormulaTypeTest_MySql)
    excel_file = pd.ExcelFile(AfterAllFormulaTypeTest_MySql)
    sheet_names = excel_file.sheet_names
    ...
    for i in range(len(sheet_formula)):
        ...
        for cell in sheet1[1]:
            if cell.coordinate in sheet1.merged_cells:
                ...
                target_cell.alignment = alignment
    workbook2.save('./AfterAllFormulaTypeTestResultWithConclusion_MySql.xlsx')
